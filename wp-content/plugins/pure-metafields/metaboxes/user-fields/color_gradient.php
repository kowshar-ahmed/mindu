<?php
/**
 * Color gradient (user meta)
 *
 * Storage shape: JSON string in user_meta (mirrors the metabox storage so
 * downstream readers can `json_decode( get_user_meta( $uid, $key, true ), true )`
 * the same way they do for post_meta). The carrier hidden input always
 * posts — empty saved + form submit results in the JSON of the current
 * UI state (or the default).
 *
 * @var string $id
 * @var string $label
 * @var mixed  $default
...
$val = array();
if ( ! empty( $_raw ) ) {
    $decoded = is_array( $_raw ) ? $_raw : json_decode( (string) $_raw, true );
...
<tr>
    ...
    <td>
        <div class="tm-meta-wrapper tpmf-user-gradient-host" style="display:block;">
        ...
        <input type="hidden"
            ...
            name="<?php echo esc_attr( $_grd_elem_id ); ?>"
            class="tm-gradient-value"
            value="<?php echo esc_attr( $_grd_json ); ?>"
        />
        </div><?php // /.tm-meta-wrapper.tpmf-user-gradient-host ?>
...
<?php ob_start(); ?>
/* TPMetaGradient.boot is defined by metaboxes/fields/color_gradient.php
   when a post-context gradient exists; on user-edit screens it may not be
   defined yet, so the same boot routine is inlined here (idempotent). */
    ...
    window.TPMetaGradient.boot = function ( wrap, hidden ) {
        ...
        function initColorPickerStop( input ) {
            ...
            jQuery( input ).wpColorPicker( {
                change: function ( ev, ui ) { jQuery( input ).val( ui.color.toString() ); syncStops(); },
